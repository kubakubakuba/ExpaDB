...
CREATE TABLE MaPrirazeno (
	id_skupina INTEGER NOT NULL REFERENCES Skupina ON DELETE CASCADE,
	id_vybaveni INTEGER NOT NULL REFERENCES Vybaveni ON DELETE CASCADE,
...
CREATE TABLE Pouziva (
	id_uzivatel INTEGER NOT NULL REFERENCES Uzivatel ON DELETE CASCADE,
	id_vybaveni INTEGER NOT NULL REFERENCES Vybaveni ON DELETE CASCADE,
...
CREATE TABLE SePrihlasujeNa (
	id_uzivatel INTEGER NOT NULL REFERENCES Ucastnik ON DELETE CASCADE,
	id_akce INTEGER NOT NULL REFERENCES Workshop ON DELETE CASCADE,
...
CREATE TABLE Organizuje (
	id_uzivatel INTEGER NOT NULL REFERENCES Vedouci ON DELETE CASCADE,
	id_akce INTEGER NOT NULL REFERENCES Workshop ON DELETE CASCADE,
	PRIMARY KEY (id_uzivatel, id_akce)
);
...
CREATE TABLE JePovinna (
	id_akce INTEGER NOT NULL REFERENCES Akce ON DELETE CASCADE,
	id_skupina INTEGER NOT NULL REFERENCES Skupina ON DELETE CASCADE,
...
CREATE TABLE JeVedoucim (
	nadrizeny INTEGER NOT NULL REFERENCES Vedouci(id_uzivatel) ON DELETE CASCADE,
	podrizeny INTEGER NOT NULL REFERENCES Vedouci(id_uzivatel) ON DELETE CASCADE,
